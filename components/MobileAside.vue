<template>
    <div class="fixed w-[100vw] z-[3000] h-[100vh] top-0 left-0 transition-all " :class="isVisible ? 'opacity-1 visible' : 'opacity-0 invisible'">
        <div @click="closeModal" class="absolute cursor-pointer top-2 left-2 w-12 h-12 bg-yellow-500 text-black font-bold z-[2000] rounded-lg flex items-center justify-center">
            <CloseIcon />
        </div>
        <div @click="closeModal" class="backdrop-blur-sm bg-stone-950/30 rounded-lg w-full h-screen z-40 top-0 right-0 fixed transition-all ease-out"></div>
        <button><i class="fa-regular fa-circle-xmark absolute left-5 top-4 z-50 text-xl text-white"></i></button>
        <div class="bg-white dark:bg-stone-900 py-10 px-5 w-[85%] h-screen fixed  top-0 z-50 block lg:hidden transition-all ease-in" :class="isVisible ? 'right-0' : 'right-[-1000px]'">
            <div class="pb-5">
                <nuxt-link to="/" class="flex items-center justify-center">
                    <svg  xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 167.67 54.24" class="w-[120px] flex items-center justify-center">
                        ...
                        <path
                            ...
                            class="cls-3" style="fill:#f58220;stroke-width:0;fill-rule:evenodd;"></path>
                        <path
                            ...
                            class="cls-1" style="fill:#f58220;stroke-width:0;fill-rule:evenodd;"></path>
                    </svg>
                </nuxt-link>
            </div>
            <ul class="flex flex-col space-y-3 mt-10">
                <nuxt-link @click="closeModal" to="/">
                    <li class="dark:text-white flex"> <Emoji stroke="#fff" width="22" height="22" /> <span class="mr-2">مرور کنید </span></li>
                </nuxt-link>
                <nuxt-link @click="closeModal" to="/favorites" v-if="authUser != null">
                    <li class="dark:text-white flex"> <Heart stroke="#fff" width="22" height="22" /> <span class="mr-2">علاقه مندی ها</span></li>
                </nuxt-link>
                <nuxt-link @click="closeModal" to="/aboutus">
                    <li class="dark:text-white flex"> <CircleInfo stroke="#fff" width="22" height="22" /> <span class="mr-2">درباره ما</span></li>
                </nuxt-link>
                <nuxt-link @click="closeModal" to="/contactus">
                    <li class="dark:text-white flex"><Phone stroke="#fff" width="22" height="22" /> <span class="mr-2">تماس با ما</span></li>
                </nuxt-link>
            </ul>
        </div>
    </div>
...
<script setup>
import CloseIcon from '@/assets/icons/svg/duelTone/close.svg'
